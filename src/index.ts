...
import { Speech, SpeechCreateParams } from './resources/speech';
import {
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['SPITCHJS_BASE_URL'].
   */
...
/**
 * API Client for interfacing with the Spitchjs API.
 */
export class Spitchjs extends Core.APIClient {
  apiKey: string;
   ...
   * API Client for interfacing with the Spitchjs API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['SPI_TCH_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['SPITCHJS_BASE_URL'] ?? https://api.spi-tch.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = Core.readEnv('SPITCHJS_BASE_URL'),
    apiKey = Core.readEnv('SPI_TCH_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.SpitchjsError(
        "The SPI_TCH_API_KEY environment variable is missing or empty; either provide it, or instantiate the Spitchjs client with an apiKey option, like new Spitchjs({ apiKey: 'My API Key' }).",
      );
    }
  ...
  static Spitchjs = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static SpitchjsError = Errors.SpitchjsError;
